...
def prepare_pdb_data_center(pdb_id, center_x, center_y, center_z, scaffold_file=None, DemoDataFolder="TamGen_Demo_Data", thr=10):
    out_split = pdb_id.lower()
    FF = glob(f"{DemoDataFolder}/*")
    for ff in FF:
        if f"gen_{out_split}" in ff:
    ...
    with open("tmp_pdb.csv", "w") as fw:
        print("pdb_id,center_x,center_y,center_z", file=fw)
        print(f"{pdb_id},{center_x},{center_y},{center_z}", file=fw)

    if scaffold_file is not None:
        script = "scripts/build_data/prepare_pdb_ids_center_scaffold.py"
        os.system(f"python {script} tmp_pdb.csv gen_{out_split} -o {DemoDataFolder} -t {thr} --scaffold-file {scaffold_file}")
    else:
        script = "scripts/build_data/prepare_pdb_ids_center.py"
        os.system(f"python {script} tmp_pdb.csv gen_{out_split} -o {DemoDataFolder} -t {thr}")
    os.system(r"rm tmp_pdb.csv")
...
class TamGenDemo:
    def __init__(
        ...
        input_args = [
            ...
            "--path",  ckpt,
            "--gen-subset", gen_subset, "--beam", "20", "--nbest", "20",
            "--max-tokens", "1024",
            "--seed", "1", 
            "--sample-beta", "1",
            "--use-src-coord", 
